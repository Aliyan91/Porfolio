        ...
        <div className="flex flex-wrap gap-2.5 mb-[22px]" role="group" aria-label="Project filters">
          {filters.map((filter) => (
            <button
              ...
              className={`min-h-[38px] px-3.5 rounded-full border font-inherit cursor-pointer transition-all hover:-translate-y-[1px] ${filter === activeFilter
          ...
          <div className="grid gap-[14px]">
            {filteredProjects.map((project) => (
              <button
                ...
                className={`w-full p-5 text-left text-inherit font-inherit cursor-pointer border rounded-lg bg-gradient-to-b from-white/[0.03] to-white/[0.01] transition-all hover:-translate-y-[1px] ${project.id === activeProject.id
